...
textOutput.insertAdjacentElement('afterend', textCursor);
...
if (currentChar < sentences[currentSentence].length) {
    if (sentences[currentSentence][currentChar] === '<') {
const endTagIndex = sentences[currentSentence].indexOf('>', currentChar);
if (endTagIndex !== -1) {
    const specialText = sentences[currentSentence].substring(currentChar, endTagIndex + 1);
    textOutput.innerHTML += specialText;
            currentChar = endTagIndex + 1;
                }
            } else {  
                const charToDisplay = sentences[currentSentence][currentChar];
                if (charToDisplay !== ' ') {
                    // 공백 문자가 아닐 때만 언더바 추가
                    textOutput.textContent += charToDisplay;
                    // textCursor.textContent = '_'; // 언더바 설정
                } else {
                    textOutput.textContent += ' '; // 공백 문자일 때는 언더바 추가하지 않음
                }
                currentChar++;
            }
...
// // 초기 페이지 로드시와 윈도우 크기가 변경될 때마다 터미널 높이를 조정
